...
def load_dataset_Train_batch(AbnormalPath, NormalPath):
    ...
    n_exp = int(n_exp)
    Num_abnormal = 99  # Total number of abnormal videos in Training Dataset.
    Num_Normal = 99  # Total number of Normal videos in Training Dataset.
...
for it_num in range(num_iters):
    ...
    if 1==1:
        ...
        rec_auc = auc(fpr, tpr)
        if(rec_auc>best_auc):
            np.save('fpr-sultani.npy',fpr)
            np.save('tpr-sultani.npy',tpr)
            best_auc=rec_auc
